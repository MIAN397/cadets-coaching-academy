...
const RevealableAmount: React.FC<{ amount: number | string }> = ({ amount }) => {
  const [revealed, setRevealed] = useState(false);
  return (
    <span 
      className="revealable-amount"
      ...
      style={{ 
        ...
        backgroundColor: revealed ? 'transparent' : 'var(--bg-slate)',
        padding: '2px 6px',
        borderRadius: '4px',
        border: revealed ? 'none' : '1px dashed var(--border-color)',
...
export const AdminDashboard: React.FC<{ adminUid: string; adminProfile: UserProfile }> = ({ adminUid, adminProfile }) => {
  ...
  const fetchBaseData = async () => {
    setLoading(true);
    try {
      // 1. Fetch all users from Firestore to build a campus map for legacy documents
      const usersSnap = await getDocs(collection(db, 'users'));
      const allUsersMap: Record<string, 'boys' | 'girls' | undefined> = {};
      const fetchedUsers: UserProfile[] = [];

      usersSnap.forEach(doc => {
        const u = doc.data() as UserProfile;
        allUsersMap[u.email.toLowerCase()] = u.campus;
        if (u.uid) {
          allUsersMap[u.uid] = u.campus;
        }

        // Only add same-campus students/teachers to the list visible to this admin
        if ((u.role === 'student' || u.role === 'teacher') && u.campus === adminProfile.campus) {
          fetchedUsers.push(u);
        }
      });
      setUsers(fetchedUsers);

      // Helper to resolve document campus
      const getDocCampus = (doc: any, type: 'quiz' | 'submission' | 'attendance'): 'boys' | 'girls' | undefined => {
        if (doc.campus) return doc.campus;
        if (type === 'quiz') {
          const email = (doc.createdByEmail || '').toLowerCase();
          return allUsersMap[email] || allUsersMap[doc.createdById];
        }
        if (type === 'submission') {
          const email = (doc.studentEmail || '').toLowerCase();
          return allUsersMap[email] || allUsersMap[doc.studentUid];
        }
        if (type === 'attendance') {
          const email = (doc.userEmail || '').toLowerCase();
          return allUsersMap[email] || allUsersMap[doc.userUid];
        }
        return undefined;
      };

      // 2. Fetch all quiz submissions and filter
      const submissionsSnap = await getDocs(collection(db, 'submissions'));
      const fetchedSubmissions: Submission[] = [];
      submissionsSnap.forEach(doc => {
        const sub = doc.data() as Submission;
        if (getDocCampus(sub, 'submission') === adminProfile.campus) {
          fetchedSubmissions.push(sub);
        }
      });
      setSubmissions(fetchedSubmissions);

      // 3. Fetch all attendance records and filter
      const attendanceSnap = await getDocs(collection(db, 'attendance'));
      const fetchedAllAttendance: AttendanceRecord[] = [];
      attendanceSnap.forEach(doc => {
        const rec = doc.data() as AttendanceRecord;
        if (getDocCampus(rec, 'attendance') === adminProfile.campus) {
          fetchedAllAttendance.push(rec);
        }
      });
      setAllAttendanceRecords(fetchedAllAttendance);

      // 4. Fetch all quizzes and filter
      const quizzesSnap = await getDocs(collection(db, 'quizzes'));
      const fetchedQuizzes: Quiz[] = [];
      quizzesSnap.forEach(doc => {
        const q = { id: doc.id, ...doc.data() } as Quiz;
        if (getDocCampus(q, 'quiz') === adminProfile.campus) {
          fetchedQuizzes.push(q);
        }
      });
  ...
  const fetchAttendanceForDate = async (dateStr: string) => {
    ...
    try {
      ...
      querySnap.forEach(doc => {
        ...
        const isSameCampus = rec.campus ? rec.campus === adminProfile.campus : users.some(u => u.email.toLowerCase() === key);
        if (key && isSameCampus) {
          records[key] = rec;
        }
      });
  ...
  // Update a single attendance status in Firestore
  const updateAttendance = async (
    ...
    setMessage(null);
    // Key attendance record by date + email
    ...
    const newRecord: AttendanceRecord = {
      ...
      loggedBy: adminUid,
      campus: targetUser.campus || adminProfile.campus,
      updatedAt: new Date()
    };
  ...
  const getUserAttendanceStats = (email: string) => {
    ...
    const userRecords = allAttendanceRecords.filter(r => 
  ...
  const handleRoleChange = async (email: string, newRole: UserRole) => {
     ...
     if (newRole === 'admin' || newRole === 'developer') {
       setMessage({ type: 'danger', text: 'Unauthorized: Admins cannot assign Admin or Developer roles.' });
       return;
     }
     try {
      const userRef = doc(db, 'users', email.toLowerCase());
      await setDoc(userRef, { role: newRole }, { merge: true });
      setMessage({ type: 'success', text: `User role reassigned to ${newRole}!` });
      setUsers(prev => prev.map(u => u.email === email ? { ...u, role: newRole } : u));
    } catch (err: any) {
      console.error(err);
      setMessage({ type: 'danger', text: 'Role update failed: ' + err.message });
    }
  };
  ...
  const handleCreateUserProfile = async (e: React.FormEvent) => {
    ...
    if (newRole === 'admin' || newRole === 'developer') {
      setMessage({ type: 'danger', text: 'Unauthorized: Admins cannot pre-register Admin or Developer profiles.' });
      setLoading(false);
      return;
    }
    ...
    try {
      const profileDoc: any = {
        uid: "", // Bind on first sign in
        email: lowerEmail,
        displayName: newName.trim(),
        role: newRole,
        password: newPassword.trim(),
        campus: adminProfile.campus,
        createdAt: serverTimestamp()
      };
      ...
      fetchBaseData(); // Reload lists
    } catch (err: any) {
      console.error(err);
      setMessage({ type: 'danger', text: 'Failed to pre-register profile: ' + err.message });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fade-in">
      <div className="dashboard-header">
        <div>
          <h2>Academy Administration Panel</h2>
          <span style={{ fontSize: '0.85rem', color: 'var(--text-light)', fontWeight: 500 }}>
            Authorize and manage attendance, cadet profiles, and roles
          </span>
        </div>
        ...
        <div className="segmented-control">
          ...
          <button 
            ...
            className={`segmented-btn ${activeTab === 'students' ? 'active' : ''}`}
          >
            <Users size={14} />
            Cadets Directory
          </button>
        ...
        <div className="fade-in">
          <div className="date-selector-row">
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <label htmlFor="attendance-date" style={{ fontSize: '0.9rem' }}>Select Target Logging Date:</label>
              <input 
                id="attendance-date"
                type="date" 
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              <button 
                onClick={() => { fetchBaseData(); fetchAttendanceForDate(attendanceDate); }} 
              ...
              <button
                onClick={() => {
                  ...
                  const filteredForAlerts = users.filter(u => {
          ...
          {workingDays.includes(attendanceDate) && (() => {
            const filteredAttendanceUsers = users.filter(u => {
              if (attFilterRole !== 'All' && u.role !== attFilterRole) return false;
              if (attSearch.trim() !== '') {
                const searchLower = attSearch.toLowerCase();
                const nameMatch = u.displayName.toLowerCase().includes(searchLower);
            ...
            return (
              <>
                ...
                <p style={{ fontSize: '0.8rem', color: 'var(--text-light)', display: 'flex', alignItems: 'center', gap: '3px' }}>
                  <Calendar size={12} />
                  <span>P = Present • A = Absent • E = Excused. Changes save immediately to Firestore.</span>
                </p>
              </>
            );
          })()}
              ...
              <table>
                ...
                <tbody>
                  {studentsList.length === 0 ? (
                    ...
                    studentsList.map(s => {
                      const { physPct, acadPct } = getUserAttendanceStats(s.email);
                      ...
                      return (
                        <tr key={s.email}>
                          ...
                          <td>
                            <input 
                              type="checkbox" 
                              checked={s.registrationDetails?.joinPhysical !== false} 
                              onChange={() => handleToggleStudentPhysicalJoin(s)} 
                          ...
                          <td>{s.registrationDetails?.phone || 'N/A'}</td>
                        </tr>
                      );
                    })
                  )}
                      ...
                      submissions.map(sub => {
                        ...
                        return (
                          <tr key={sub.id}>
                            ...
                            <td>{sub.quizTitle}</td>
                            <td>
                              <span style={{ fontWeight: 'bold' }}>{sub.score}</span> / {sub.totalQuestions}
                            </td>
                            <td>
                              <span 
                                style={{ 
      ...
      {/* 3. ROLES ASSIGNMENT & PRE-REGISTRATION */}
      {activeTab === 'roles' && (
        <div className="fade-in">
          ...
          <div style={{ backgroundColor: 'var(--bg-white)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '2rem', marginBottom: '2rem', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: 'var(--text-light)', marginBottom: '1.5rem' }}>
              Create a user's dossier profile first. The student/teacher can then sign in with their email to activate their account and set their password.
            </p>
                                        ...
                                        <div key={quiz.id} style={{ backgroundColor: 'var(--bg-white)', border: '1px solid var(--border-color)', borderRadius: '6px', overflow: 'hidden' }}>
                                          ...
                                          <div style={{ padding: '0.75rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                                            <div>
                                              ...
                                              <div style={{ display: 'flex', gap: '0.75rem', fontSize: '0.8rem', color: 'var(--text-light)', marginTop: '2px', flexWrap: 'wrap' }}>
                        ...
                        {reminderStudents.map(student => {
                          ...
                          return (
                            <tr key={student.email}>
                              ...
                              <td>{student.email}</td>
                               <td style={{ fontWeight: 700 }}><RevealableAmount amount={effective.amount} /></td>
                        ...
                        return (
                          <tr key={student.email}>
                            ...
                            <td style={{ fontWeight: 700 }}>
                              <RevealableAmount amount={effective.amount} />
                            </td>
